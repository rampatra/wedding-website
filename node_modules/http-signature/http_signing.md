...
mechanisms, and RFC5246 (TLS 1.2) defines user-auth, both of which are widely
...
The "signature" authentication scheme is based on the model that the user must
...
The user is expected to send an Authorization header (as defined in RFC 2617)
...
REQUIRED. The `algorithm` parameter is used if the user and server agree on a
...
generated by the user. The user uses the `algorithm` and `headers` request
...
In order to generate the string that is signed with a key, the user MUST take
...
The "rsa-key-1" keyId refers to a private key known to the user and a public
...
The user would compose the signing string as:
