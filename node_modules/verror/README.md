...
and user interface, and clients make RPC requests to servers.  Let's say the
server fails with an UnauthorizedError with message "user 'bob' is not
authorized".  The user wraps all server errors with a FastServerError.  The
user also wraps all request errors with a FastRequestError that includes the
...
* The caller can tell that the request failed on the server, rather than as a
  result of a user problem (e.g., failure to serialize the request), a
