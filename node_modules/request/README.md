...
At this point, the connection is left open, and the user is
...
var fs = require('fs')
    , path = require('path')
    , certFile = path.resolve(__dirname, 'ssl/user.crt')
    , keyFile = path.resolve(__dirname, 'ssl/user.key')
...
In the example below, we call an API that requires user side SSL certificate
...
timeouts**. A connect timeout occurs if the timeout is hit while your user is
